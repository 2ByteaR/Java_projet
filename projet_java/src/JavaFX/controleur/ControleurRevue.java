...
public class ControleurRevue implements Initializable{
    ...
    public void refreshTable(){
        ...
        this.tblRevue.getItems().addAll(DAOFactory.getDAOFactory(Persistance.MYSQL).getRevueDAO().findAll());
    }
        ...
        this.tblRevue.getSelectionModel().selectedItemProperty
                ...
                (observable, oldValue, newValue) -> {
                    ...
                    if (!(revue == null)) {
                        ...
                        cbxPeriodicite.getSelectionModel().select((Periodicite) DAOFactory.getDAOFactory(Persistance.MYSQL).getPeriodicteDAO().getById(revue.getId_periodicite()));
                    }
                });
    ...
    public void ajouterRevue(ActionEvent event) throws IOException {
        ...
        if (verificationDonnee()) {
            ...
            Revue revue = new Revue(0, periodicite.getId(), textDescriptionRevue.getText(), tarif, textTitreRevue.getText(), "bonjour");
            DAOFactory.getDAOFactory(Persistance.MYSQL).getRevueDAO().create(revue);
        }
    ...
    void supprimerRevue(ActionEvent event) {
        DAOFactory.getDAOFactory(Persistance.MYSQL).getRevueDAO().delete(tblRevue.getSelectionModel().getSelectedItem());
        refreshTable();
    }
    ...
    void modifierRevue(ActionEvent event) {
        if (verificationDonnee()){
            ...
            DAOFactory.getDAOFactory(Persistance.MYSQL).getRevueDAO().update(new Revue(revue.getId_revue(), periodicite.getId(), textDescriptionRevue.getText(), tarif, textTitreRevue.getText(), "bonjour"));
            refreshTable();
        }
    }
